...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="textBoxWidth" select="150"/>
  ...
  <xsl:template match="*" mode="buffer">
    <xsl:copy>
      <xsl:copy-of select="@*"/>
      <xsl:variable name="brood" select="dsk:e | dsk:text"/>
  ...
  <xsl:template match="dsk:label" mode="draw">
    ...
    <text font-size="12" fill="black" x="{$depth + 2}" y="{$offset - 2} ">
      <xsl:apply-templates mode="#current"/>
    </text>
  </xsl:template>
  ...
  <xsl:template match="dsk:e | dsk:text" mode="draw">
    ...
    <xsl:variable name="extent" as="xs:double"><!-- marker extent -->
      <xsl:choose>
        <xsl:when test="exists(dsk:label)">
          <xsl:sequence select="dsk:label/@extent"/>
        </xsl:when>
        <xsl:when test="@extent = 1">
          <xsl:sequence select="$textBoxWidth div 2"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:sequence select="$textBoxWidth"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
  ...
  <xsl:template match="dsk:e[exists(dsk:label)]" priority="99" mode="box-style">
    <xsl:attribute name="stroke">gold</xsl:attribute>
    <xsl:attribute name="fill">gold</xsl:attribute>
    <xsl:attribute name="fill-opacity">0.3</xsl:attribute>
  </xsl:template>

  <xsl:template match="dsk:e[@gi='teiHeader']" mode="box-style">
    <xsl:attribute name="fill">gold</xsl:attribute>
  </xsl:template>
  ...
  <xsl:template match="dsk:e[@gi='p']" mode="box-style">
    <xsl:attribute name="fill">lavender</xsl:attribute>
  </xsl:template>
  
  <xsl:template match="dsk:e[@gi='listBibl']" mode="box-style">
    <xsl:attribute name="stroke">darkred</xsl:attribute>
    <xsl:attribute name="fill">pink</xsl:attribute>
  </xsl:template>
  
  <xsl:template match="dsk:e[@gi='div']" mode="box-style">
    <xsl:attribute name="stroke-dasharray">2 1</xsl:attribute>
  </xsl:template>
  
  <xsl:template match="dsk:e[@gi='floatingText']" mode="box-style">
    <xsl:attribute name="fill">khaki</xsl:attribute>
  </xsl:template>

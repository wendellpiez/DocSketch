...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
  ...
  <xsl:variable name="published" select="$assigned-issue and not($assigned-issue/@preview)"/>
    ...
    <xsl:template name="article_main_body" match="tei:TEI">
      ...
      <xsl:choose>
        <xsl:when test="false() and $aprilfool = true()">
          <xsl:call-template name="pubInfo"/>
          <xsl:apply-templates select="tei:teiHeader"/>
          <iframe style="margin-top:2em; margin-bottom:2em; width:100%;height:400px">
    ...
    <xsl:template name="toolbar">
        <div class="toolbar">
                ...
                <xsl:choose>
                    <xsl:when test="$published">
                        <xsl:attribute name="href">
                            <xsl:value-of select="concat('/dhq/vol/',$vol_no_zeroes,'/',$issue,'/index.html')"/>
                        </xsl:attribute>
            ...
            <a rel="external">
                <xsl:attribute name="href">
                    <xsl:value-of select="concat('/dhq/vol/',$vol_no_zeroes,'/',$issue,'/',$id,'.xml')"/>
                </xsl:attribute>
...
|&#x00a0;
        ...
        <div class="author">
            <a rel="external">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:attribute name="href">
                            <xsl:value-of select="concat('/dhq/preview/',$bioFile,'#',$bios)"/> 
                        </xsl:attribute>
                    </xsl:otherwise>
                </xsl:choose>
    ...
    <xsl:template match="dhq:authorInfo/tei:email" mode="author">
...
<!-- Make sure the blank abstracts do not appear -->
    ...
    <xsl:template match="tei:p">
        ...
        <xsl:choose>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                      <xsl:variable name="counter">
                            <xsl:number count="/tei:TEI/tei:text/tei:body//tei:p[not(ancestor::dhq:example or ancestor::tei:figure or ancestor::tei:quote or parent::tei:note or ancestor::tei:sp[ancestor::tei:eg or ancestor::tei:list or ancestor::tei:quote[@rend='block']])]"
                                from="tei:TEI/tei:text/tei:body" level="any"/>
                      </xsl:variable>
                        <div class="counter">
                          <a href="#p{$counter}">
            ...
            <xsl:otherwise>
              <xsl:variable name="counter">
                        <xsl:number count="/tei:TEI/tei:text/tei:body//tei:p[not(ancestor::dhq:example or ancestor::tei:figure or ancestor::tei:quote or parent::tei:note or ancestor::tei:sp[ancestor::tei:epigraph or ancestor::tei:quote[@rend='block']])]"
                            from="tei:TEI/tei:text/tei:body" level="any"/>
              </xsl:variable>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise><div class="ptext" id="p{$counter}"><xsl:apply-templates/></div></xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
  ...
  <xsl:template name="assign-class">
    ...
    <xsl:template match="tei:table/tei:head" mode="label">
        <xsl:text>Table&#160;</xsl:text>
        <xsl:number count="tei:table" level="any" from="/"/>
    </xsl:template>
    
    <xsl:template match="dhq:example/tei:head" mode="label">
        <xsl:text>Example&#160;</xsl:text>
        <xsl:number count="//dhq:example/tei:head" level="any" from="/"/>
    </xsl:template>

    <xsl:template match="tei:figure" mode="label">
        <xsl:text>Figure&#160;</xsl:text>
        <xsl:number count="tei:figure[not(ancestor::tei:table)]" level="any" from="/"/>
    </xsl:template>
  ...
  <xsl:template match="tei:listBibl/tei:bibl">
    <div class="bibl">
      <xsl:call-template name="show-bibl"/>
    </div>
  </xsl:template>
  ...
  <xsl:template name="show-bibl">
    ...
    <!-- Any whitespace-only text will be dropped if it doesn't have a
         ...
         similarly -->
    <xsl:template mode="scrubbing" match="text()[not(normalize-space())]
